defmodule Robotica.Plugins.LIFX do
  ...
  # @spec turn_on(State.t(), boolean()) :: :ok | {:error, String.t()}
  # defp turn_on(%State{} = state, force_brightness_off) do
  ...
  #   set_off_color = fn power ->
  #     if power == 0 or force_brightness_off do
  #       set_color_wait(state, color_off, 0)
  #     else
  #       :ok
  #     end
  #   end
  ...
  #        Logger.debug("#{device_to_string(state, nil)}: Turn on power #{power}."),
  #        :ok <- set_off_color.(power),
  #        :ok <- on_wait(state) do
  #     :ok
  #   else
  #     {:error, err} -> {:error, err}
  ...
  #          :ok <- set_color(state, light, command, state.config, 0, 0),
  #          Process.sleep(400),
  #          IO.inspect("restoring state #{inspect light_state}"),
